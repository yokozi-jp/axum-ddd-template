...
src/
├── features/
│   ├── {feature}/             # e.g., user, order, payment
│   │   ├── domain/
...
│   ├── domain/
│   │   ├── error               # Common domain errors
│   │   └── value_objects       # Money, DateTime wrappers
│   ├── application/
...
│
└── main                        # Bootstrap / DI composition root
```

**Package by Feature Rules:**
- Each feature is self-contained with its own domain/application/infrastructure
- Features reference other features by ID only (e.g., Order has UserId, not User entity)
- `features/{feature}/domain` has ZERO dependencies on other features
- Shared logic goes in `shared/` (common value objects, infrastructure utilities)
